...
# 搞两个全局变量指定用例文件及接口名称
filename = os.path.join(CASEYAML, "test_009_emergcase_uploads.yml")
api_name = "emergcase_uploads"


@allure.epic("指挥中心应急事件上报")
class TestEmergcaseUploads:

    @allure.title("指挥中心应急事件上报-市级")
    @pytest.mark.api
    # @pytest.mark.run(order=1)
    def test_emergcase_uploads_0(cls):
        case_index = 0
        # my_request = AllApis()
    ...
    def test_emergcase_uploads_1(cls):
        case_index = 1
        # my_request = AllApis()
        method, url, data, username, password, status_code, real_status_code, dict_return_data = AllApis().yaml_api_requests(
            filename=filename, api_name=api_name, case_index=case_index)
    ...
    def test_emergcase_uploads_2(cls):
        case_index = 2
        # my_request = AllApis()
        method, url, data, username, password, status_code, real_status_code, dict_return_data = AllApis().yaml_api_requests(
            filename=filename, api_name=api_name, case_index=case_index)
