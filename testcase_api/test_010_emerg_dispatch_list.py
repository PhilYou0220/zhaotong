...
# 搞两个全局变量指定用例文件及接口名称
filename = os.path.join(CASEYAML, "test_010_emerg_dispatch_list.yml")
api_name = "emerg_dispatch_list"


@allure.epic("指挥中心应急事件上报")
class TestEmergDispatchList:

    @allure.title("指挥中心应急事件上报-市级")
    @pytest.mark.api
    @pytest.mark.run(order=1)
    def test_emerg_dispatch_list_0(cls):
        case_index = 0
        # my_request = AllApis()
    ...
    @allure.title("指挥中心应急事件上报--区县")
    @pytest.mark.api
    @pytest.mark.run(order=2)
    def test_emerg_dispatch_list_1(cls):
        case_index = 1
        # my_request = AllApis()
        method, url, data, username, password, status_code, real_status_code, dict_return_data = AllApis().yaml_api_requests(
            filename=filename, api_name=api_name, case_index=case_index)
    ...
    @allure.title("指挥中心应急事件上报--镇街")
    @pytest.mark.api
    @pytest.mark.run(order=3)
    def test_emerg_dispatch_list_2(cls):
        case_index = 2
        # my_request = AllApis()
        method, url, data, username, password, status_code, real_status_code, dict_return_data = AllApis().yaml_api_requests(
            filename=filename, api_name=api_name, case_index=case_index)
