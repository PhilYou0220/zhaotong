...
# 搞两个全局变量指定用例文件及接口名称
filename = os.path.join(CASEYAML, "test_083_boss_car_inout_data_weight_summary_data_export.yml")
api_name = "boss_car_inout_data_weight_summary_data_export"


@allure.epic("矿山数据统计--加工销售数据--销售量导出")
class TestMoatOrgFullTree:

    @allure.title("矿山数据统计--加工销售数据--销售量导出")
    @pytest.mark.api
    @pytest.mark.run(order=1)
    def test_moat_org_full_tree_0(cls):
        case_index = 0
        method, url, data, username, password, status_code, real_status_code, dict_return_data = AllApis().yaml_api_requests_export(
    ...
    @allure.title("矿山数据统计--加工销售数据统计列表筛选--销售量导出")
    @pytest.mark.api
